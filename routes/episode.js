...
router.get("/:podcast_id/epsiode", async (req, res) => {
  try {
    const result = await Podcast.findById(req.params.podcast_id);
...
router.get("/:episode_id", async (req, res) => {
  try {
    const result = await Podcast.findById(req.params.episode_id);
...
router.delete("/:episodeID", async (req, res) => {
  try {
    ...
    const id_finder = await Podcast.find({
    ...
    const deletePodcast = await Podcast.updateMany(
      { _id: get_podcast_id },
...
router.put("/:episode_id", async (req, res) => {
  try {
    ...
    const edit_episode = await Podcast.updateMany(
      {
